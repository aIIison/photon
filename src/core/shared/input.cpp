...
void photon::c_input::poll_input( ) {
...
bool photon::c_input::get_key_held( unsigned int code ) {
	return cur_keys[ code ];
}

bool photon::c_input::get_key_release( unsigned int code ) {
	return !cur_keys[ code ] && old_keys[ code ];
}

bool photon::c_input::get_key_press( unsigned int code ) {
	return cur_keys[ code ] && !old_keys[ code ];
}

vec2_t photon::c_input::get_cursor_position( ) {
	return { cursor_x, cursor_y };
}

bool photon::c_input::is_cursor_in_area( int x0, int y0, int x1, int y1 ) {
